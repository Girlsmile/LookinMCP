...
@property(nonatomic, strong) NSPopover *mcpPopover;
...
- (instancetype)init {
    ...
    if (self = [self initWithWindow:window]) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(_handleMCPStatusDidChange) name:LKMCPHostManagerDidUpdateNotification object:nil];
        [[LKMCPHostManager sharedInstance] startHost];
        [self _refreshMCPToolbarItem];
    }
    return self;
}
...
- (nullable NSToolbarItem *)toolbar:(NSToolbar *)toolbar itemForItemIdentifier:(NSToolbarItemIdentifier)itemIdentifier willBeInsertedIntoToolbar:(BOOL)flag {
    ...
    if (!item) {
        ...
        } else if ([item.itemIdentifier isEqualToString:LKToolBarIdentifier_MCP]) {
            item.target = self;
            item.action = @selector(_handleMCP:);
            [self _refreshMCPToolbarItem];
        }
    }
    return item;
...
- (void)_refreshMCPToolbarItem {
    NSToolbarItem *item = self.toolbarItemsMap[LKToolBarIdentifier_MCP];
    ...
    NSString *text = [NSString stringWithFormat:@"● %@", @"MCP"];
    NSMutableAttributedString *title = [[NSMutableAttributedString alloc] initWithString:text];
    [title addAttribute:NSForegroundColorAttributeName value:manager.statusColor range:NSMakeRange(0, title.length)];
    [title addAttribute:NSFontAttributeName value:[NSFont systemFontOfSize:12 weight:NSFontWeightSemibold] range:NSMakeRange(0, title.length)];
    button.attributedTitle = title;
    button.toolTip = [NSString stringWithFormat:@"MCP %@\n%@", manager.statusText, manager.statusSummaryText ?: @""];
}

- (void)_handleMCP:(NSButton *)button {
    if (self.mcpPopover.isShown) {
        [self.mcpPopover close];
        return;
    }
    LKMCPStatusPopoverController *vc = [[LKMCPStatusPopoverController alloc] init];
    NSPopover *popover = [[NSPopover alloc] init];
    popover.behavior = NSPopoverBehaviorTransient;
    popover.animates = NO;
    popover.contentSize = [vc contentSize];
    popover.contentViewController = vc;
    self.mcpPopover = popover;
...
/// 当前 hierarchy 或 detail 进入稳定态后，刷新一份本地 snapshot 给 MCP 读取。
- (void)_exportSnapshotIfPossible {
    NSError *error = nil;
    if (![[LKExportManager sharedInstance] exportCurrentSnapshotWithError:&error] && error) {
        NSLog(@"LookinMCP snapshot export failed: %@", error.localizedDescription);
    }
}
