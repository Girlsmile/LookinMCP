...
NSToolbarItemIdentifier const LKToolBarIdentifier_MCP = @"20";
...
- (NSToolbarItem *)makeToolBarItemWithIdentifier:(NSToolbarItemIdentifier)identifier preferenceManager:(LKPreferenceManager *)manager {
    ...
    if ([identifier isEqualToString:LKToolBarIdentifier_MCP]) {
        NSButton *button = [NSButton new];
        button.title = @"MCP";
        button.bezelStyle = NSBezelStyleTexturedRounded;
        button.font = [NSFont systemFontOfSize:12 weight:NSFontWeightSemibold];
        
        NSToolbarItem *item = [[NSToolbarItem alloc] initWithItemIdentifier:LKToolBarIdentifier_MCP];
        item.label = @"MCP";
        item.view = button;
        item.minSize = NSMakeSize(72, 34);
        item.maxSize = item.minSize;
        return item;
    }
